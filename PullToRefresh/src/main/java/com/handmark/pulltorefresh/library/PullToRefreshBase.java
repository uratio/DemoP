...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	protected void onScrollChanged(int l, int t, int oldl, int oldt) {
		...
		Log.d(LOG_TAG, "onScrollChanged: l"+l+", t"+t+", oldl"+oldl+", oldt" + oldt);
	}
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		if (!isPullToRefreshEnabled()) {
			Log.d(LOG_TAG, "onInterceptTouchEvent ____ 1");
			return false;
		}

		final int action = event.getAction();
		Log.d(LOG_TAG, "onInterceptTouchEventaction="+action);

		if (action == MotionEvent.ACTION_CANCEL || action == MotionEvent.ACTION_UP) {
			Log.d(LOG_TAG, "onInterceptTouchEvent ____ 2");
			mIsBeingDragged = false;
			return false;
		}

		if (action != MotionEvent.ACTION_DOWN && mIsBeingDragged) {
			Log.d(LOG_TAG, "onInterceptTouchEvent ____ 3");
			return true;
		}

		switch (action) {
			case MotionEvent.ACTION_MOVE: {
				Log.d(LOG_TAG, "onInterceptTouchEvent ____ event.getY()="+event.getY());
				Log.d(LOG_TAG, "onInterceptTouchEvent ____ mScrollingWhileRefreshingEnabled="+mScrollingWhileRefreshingEnabled);
				Log.d(LOG_TAG, "onInterceptTouchEvent ____ isRefreshing()="+isRefreshing());
				// If we're refreshing, and the flag is set. Eat all MOVE events
				if (!mScrollingWhileRefreshingEnabled && isRefreshing()) {
					return true;
				}

				Log.d(LOG_TAG, "onInterceptTouchEvent ____ isReadyForPull()="+isReadyForPull());
				if (isReadyForPull()) {
					...
					Log.d(LOG_TAG, "onInterceptTouchEvent ____ absDiff="+absDiff);
					Log.d(LOG_TAG, "onInterceptTouchEvent ____ mTouchSlop="+mTouchSlop);
					Log.d(LOG_TAG, "onInterceptTouchEvent ____ mFilterTouchEvents="+mFilterTouchEvents);
					Log.d(LOG_TAG, "onInterceptTouchEvent ____ Math.abs(oppositeDiff))="+Math.abs(oppositeDiff));

					if (absDiff > mTouchSlop && (!mFilterTouchEvents || absDiff > Math.abs(oppositeDiff))) {
						Log.d(LOG_TAG, "onInterceptTouchEvent ____ mMode.showHeaderLoadingLayout()="+mMode.showHeaderLoadingLayout());
						Log.d(LOG_TAG, "onInterceptTouchEvent ____ diff="+diff);
						Log.d(LOG_TAG, "onInterceptTouchEvent ____ isReadyForPullStart()="+isReadyForPullStart());
		...
		Log.d(LOG_TAG, "onInterceptTouchEvent ____ 4  mIsBeingDragged="+mIsBeingDragged);
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		if (!isPullToRefreshEnabled()) {
			Log.d(LOG_TAG, "onTouchEvent ____ 1");
			return false;
		}

		// If we're refreshing, and the flag is set. Eat the event
		if (!mScrollingWhileRefreshingEnabled && isRefreshing()) {
			Log.d(LOG_TAG, "onTouchEvent ____ 2");
			return true;
		}

		if (event.getAction() == MotionEvent.ACTION_DOWN && event.getEdgeFlags() != 0) {
			Log.d(LOG_TAG, "onTouchEvent ____ 3");
			return false;
		}
		...
		switch (event.getAction()) {
			case MotionEvent.ACTION_MOVE: {
				if (mIsBeingDragged) {
					...
					pullEvent();
					Log.d(LOG_TAG, "onTouchEvent ____ 4");
					return true;
				}
				break;
			}
			...
			case MotionEvent.ACTION_DOWN: {
				if (isReadyForPull()) {
					mLastMotionY = mInitialMotionY = event.getY();
					mLastMotionX = mInitialMotionX = event.getX();
					Log.d(LOG_TAG, "onTouchEvent ____ 5");
					return true;
				}
				break;
			}
	...
	protected final void setHeaderScroll(int value) {
		...
		//当视图滚动不是手指移动引起时，刷新头部不显示
		if (mState == State.OVERSCROLLING)return;
	...
	private boolean isReadyForPull() {
		Log.d(LOG_TAG, "isReadyForPull ____ mMode="+mMode+'\t'+""+getScrollY());
		switch (mMode) {
			case PULL_FROM_START:
...
//				return true;
	...
	private void pullEvent() {
		...
		Log.d(LOG_TAG, "newScrollValue: " + newScrollValue+'\t'+"isRefreshing="+isRefreshing());
		if (newScrollValue != 0 && !isRefreshing()) {
			float scale = Math.abs(newScrollValue) / (float) itemDimension;
			switch (mCurrentMode) {
				case PULL_FROM_END:
					Log.d(LOG_TAG, "*****PULL_FROM_END: ");
					mFooterLayout.onPull(scale);
					break;
				case PULL_FROM_START:
				default:
					Log.d(LOG_TAG, "*****PULL_FROM_START: ");
